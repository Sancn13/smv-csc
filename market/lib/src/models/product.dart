...
class Product {
  ...
  Product.fromJSON(Map<String, dynamic> jsonMap) {
    try {
      ...
      options = jsonMap['options'] != null && (jsonMap['options'] as List).length > 0 ? List.from(jsonMap['options']).map((element) => Option.fromJSON(element)).toSet().toList() : [];
  ...
  Coupon _couponDiscountPrice(Coupon coupon) {
    coupon.valid = true;
    discountPrice = price;
    for(var i=0;i<coupon.discountPrice.length;i++){
      if(id == coupon.discountPrice[i].product_id){
        price = coupon.discountPrice[i].price;
      }
    }
    if (price < 0) price = 0;
    return coupon;
