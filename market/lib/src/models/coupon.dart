import 'price_product_discount.dart';
...
class Coupon {
  ...
  bool enabled;
  List<PriceProductDiscount> discountPrice;
  ...
  Coupon.fromJSON(Map<String, dynamic> jsonMap) {
    try {
      ...
      discountPrice = jsonMap['price_product'] != null ? List.from(jsonMap['price_product']).map((element) => PriceProductDiscount.fromJSON(element)).toList() : [];
      valid = jsonMap['valid'];
    } catch (e) {
      id = '';
      code = '';
