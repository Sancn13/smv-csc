...
class TrackingController extends ControllerMVC {
  ...
  void listenForOrderStatus() async {
    final Stream<OrderStatus> stream = await getOrderStatus(order.id,order.payment.method);
  ...
  List<Step> getTrackingSteps(BuildContext context) {
    ...
    this.orderStatus.forEach((OrderStatus _orderStatus) {
        if(order.deliveryStatus == _orderStatus.status){
          order.deliveryStatusId = _orderStatus.id;
        }
      _orderStatusSteps.add(Step(
        ...
        isActive: (int.tryParse(order.deliveryStatusId)) >= (int.tryParse(_orderStatus.id)),
      ));
    });
