...
class ProductController extends ControllerMVC {
  ...
  void addToCart(Product product, {bool reset = false}) async {
    ...
    _newCart.options = product.options.where((element) => element.checked == true).toList();
    _newCart.product.optionGroups = product.optionGroups;
  ...
  void addToFavorite(Product product) async {
   ...
   void removeFromFavorite(Favorite _favorite,String product_id) async {
    removeFavorite(_favorite,product_id).then((value) {
      setState(() {
