...
class _ProductWidgetState extends StateMVC<ProductWidget> {
  ...
  void getValue(){
    if(load == true){
      list_options = [];
      optionss = [];
      print(_con.product.optionGroups.length);
      for(int i=0;i<_con.product.optionGroups.length;i++){
        List<String> temp = [];
        for(int j=0;j<_con.product.options.length;j++){
          if(_con.product.options[j].optionGroupId == _con.product.optionGroups[i].id){
            temp.add(_con.product.options[j].name);
          }
        }
        optionss.add(temp);
        list_options.add(temp[0]);
      }
    }
    setState(() {
      load = false;
  ...
  void getOptions(){
    ...
    for(int i =0;i<_con.product.options.length;i++){
      ...
      for(int j = 0;j<list_options.length;j++){
        if(_con.product.options[i].name == list_options[j]){
                                              ...
                                              ExpansionTile(
                                                ...
                                                children: [
                                                  DropdownButton<String>(
                                                    value: list_options[optionGroupIndex],
                                                    ...
                                                    onChanged: (String newValue) {
                                                      setState(() {
                                                        list_options[optionGroupIndex] = newValue;
                                                        getOptions();
                                                      });
                                                    },
                                                    items: optionss[optionGroupIndex].map<DropdownMenuItem<String>>((String value) {
                              ...
                              children: <Widget>[
                                Expanded(
                                  child: _con.favorite?.id != null
                                      ? OutlineButton(
                                          onPressed: () {
                                            _con.removeFromFavorite(_con.favorite,widget.routeArgument.id);
                                          },
